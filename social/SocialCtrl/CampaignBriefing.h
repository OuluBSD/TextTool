#ifndef _SocialTool_CampaignBriefing_h_
...
class CampaignBriefing : public ToolAppCtrl {
	...
	#define ALBUM_BRIEFING_LIST \
		ITEM(GENERAL_HISTORY, t_("General History"), t_("Common factors in the history of programs")) \
		...
		ITEM(LIMITS_BEGIN, t_("Limitations of the beginning"), t_("Significant events that happened immediately before all programs in this album")) \
		ITEM(LIMITS_END, t_("Limitations of the end"), t_("Significant events that happened immediately after all programs in this album")) \
		...
		ITEM(LIFE_ART, t_("Art of the Author"), t_("What kind of art did the company make?")) \
		...
		ITEM(THEME_COMPOSITION, t_("Composition Theme"), t_("Common musical style of programs")) \
		ITEM(THEME_LYRICAL, t_("Lyrical Theme"), t_("Common lyrical style of programs")) \
		ITEM(THEME_PRODUCTION, t_("Production Theme"), t_("Common genre and production limitations and features of programs")) \
...
public:
	typedef CampaignBriefing CLASSNAME;
	CampaignBriefing();
