...
void ScriptEditor::Data() {
	...
	for(const StaticPart* spp : parts) {
		...
		for(int j = 0; j < lines.GetCount(); j++) {
			...
			String saved;
			if (j < sp.saved_scripts.GetCount())
				saved = sp.saved_scripts[j];
...
void ScriptEditor::ToolMenu(Bar& bar) {
...
void ScriptEditor::CopyPronounciationToClipboard() {
...
void ScriptEditor::SetPhrase() {
	...
	if (line_i >= sp.saved_scripts.GetCount())
		sp.saved_scripts.SetCount(line_i+1);
	
	AttrText at = suggestions.Get(0);
	String txt = at.text.ToString();
	sp.saved_scripts[line_i] = txt;
	
	phrase_list.Set(phrase_i, 2, txt);
	PostCallback(THISBACK1(MovePhrase, 1));
}

void ScriptEditor::MovePhrase(int i) {
	if (!phrase_list.IsCursor())
		return;
...
void ScriptEditor::GetSuggestionsAI() {
	...
	m.GetScriptPhrase(args, THISBACK(OnSuggestions));
}
#endif

void ScriptEditor::OnSuggestions(String res) {
