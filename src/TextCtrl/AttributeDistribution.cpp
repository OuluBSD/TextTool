...
void AttributeDistribution::Data() {
	...
	if (!db.ctx.ed.artist || !db.ctx.ed.release || !db.ctx.ed.song)
		return;
	Entity& artist = *db.ctx.ed.artist;
	Snapshot& release = *db.ctx.ed.release;
	Component& song = *db.ctx.ed.song;
...
void AttributeDistribution::DataSuggestions() {
	...
	for(int da_i = 0; da_i < sda.datasets.GetCount(); da_i++) {
		...
		for(int i = 0; i < da.entities.GetCount(); i++) {
			...
			const String& artist_name = da.entities.GetKey(i);
			for(int j = 0; j < artist.components.GetCount(); j++) {
				...
				for(int k = 0; k < song.positive_roles.GetCount(); k++) {
					Vector<ScriptAnalysis::Role>& v = song.positive_roles[k];
				...
				for(int k = 0; k < song.negative_roles.GetCount(); k++) {
					Vector<ScriptAnalysis::Role>& v = song.negative_roles[k];
					if (v.GetCount() < 2) continue;
					/*bool is_partially_added = false;
					for (const ScriptAnalysis::Role& r : v) {
						int l = active_attrs.Find(r.group);
