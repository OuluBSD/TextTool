...
void LineStructureEditor::Data() {
	...
	ComponentCandidateCache& cache = da.song_cache.GetAdd(title);
	cache.Realize(song);
	
	
	for(int i = 0; i < song.parts.GetCount(); i++) {
		StaticPart& sp = song.parts[i];
		ComponentCandidateCache::Part& part = cache.parts[i];
		Color clr = GetComponentPartPaperColor(sp.type);
...
void LineStructureEditor::DataPart() {
	...
	ComponentCandidateCache::Part& part = cache.parts[part_i];
