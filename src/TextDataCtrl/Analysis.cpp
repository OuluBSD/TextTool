...
void TextDataAnalysisPage::OnRhymes(String res, int list_i, bool start_next) {
	...
	Task& t = tasks[list_i];
	ScriptDataset& ld = *t.ld;
	ScriptAnalysis& anal = *t.analysis;
...
void TextDataAnalysisPage::OnRhymeLocations(String res, int list_i, bool start_next) {
	...
	for (String& line : lines) {
		...
		if (words.GetCount()) {
			ScriptAnalysis::RhymeLocationLine& l = anal.rhyme_locations.Add();
			l.words <<= words;
			l.rhyming <<= rhyming;
		}
	}
...
void TextDataAnalysisPage::UpdateTasks() {
	...
	for (int ds_i = 0; ds_i < sd.GetCount(); ds_i++) {
		...
		for(int i = 0; i < ds.entities.GetCount(); i++) {
			String artist_name = ds.entities.GetKey(i);
			auto& v = ds.entities[i].components;
			for(int j = 0; j < v.GetCount(); j++) {
				ScriptAnalysis& song = v[j];
				bool found = false;
				
				if (!found) for(EntityDataset& ad : sd.entities_en) {
					if (ad.name != artist_name) continue;
					for (ScriptDataset& ld : ad.scripts) {
				...
				if (!found) for(EntityDataset& ad : sd.entities_fi) {
					if (ad.name != artist_name) continue;
					for (ScriptDataset& ld : ad.scripts) {
						if (ld.name != song.name) continue;
						if (ld.text.GetCount() > txt_limit) continue;
