...
void TaskMgr::GetBiography(const BiographyProcessArgs& args, Event<String> WhenResult) {
	const TaskMgrConfig& mgr = TaskMgrConfig::Single();
	TaskMgr& p = *this;
	...
	task_lock.Enter();
	AiTask& t = tasks.Add();
	t.SetRule(AITASK_BIOGRAPHY_PROCESS, MakeName(args, -1, "biography process"))
...
void TaskMgr::GetBiographySummary(const BiographySummaryProcessArgs& args, Event<String> WhenResult) {
	...
	task_lock.Enter();
	AiTask& t = tasks.Add();
	t.SetRule(AITASK_BIOGRAPHY_SUMMARY_PROCESS, MakeName(args, -1, "biography summary process"))
...
void TaskMgr::GetCode(const CodeArgs& args, Event<String> WhenResult) {
	...
	task_lock.Enter();
	AiTask& t = tasks.Add();
	t.SetRule(AITASK_CODE, MakeName(args, -1, "code"))
...
void TaskMgr::GetSnapshot(const SnapshotArgs& args, Event<String> WhenResult) {
	const TaskMgrConfig& mgr = TaskMgrConfig::Single();
	TaskMgr& p = *this;
	...
	task_lock.Enter();
	AiTask& t = tasks.Add();
	t.SetRule(AITASK_SNAPSHOT, MakeName(args, -1, "snapshot"))
		.Input(&AiTask::CreateInput_Snapshot)
		.Process(&AiTask::Process_Default);
...
void TaskMgr::GetVideoSolver(const VideoSolverArgs& args, Event<String> WhenResult) {
	const TaskMgrConfig& mgr = TaskMgrConfig::Single();
	TaskMgr& p = *this;
	...
	task_lock.Enter();
	AiTask& t = tasks.Add();
	t.SetRule(AITASK_SNAPSHOT, MakeName(args, -1, "video solver"))
		.Input(&AiTask::CreateInput_VideoSolver)
		.Process(&AiTask::Process_Default);
